...
function migrateFromJSON(d) {
  const run = db.transaction(() => {
    ...
    // Subgroups
    for (const sg of (d.subgroups || [])) {
      db.prepare(`INSERT OR REPLACE INTO subgroups (id, nom, categoryId, levelId, groupType, days, time, startDate, endDate, sessionsPerCycle, price)
                  VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`)
    ...
    for (const st of (d.students || [])) {
      const pw = st.password || hashPasswordSync("000000");
      db.prepare(`INSERT OR REPLACE INTO students (id, nom, prenom, phone, password, subgroupId, nfcCardId, enrollmentPaid, enrollmentDate, lastModified, createdAt)
                  VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`)
    ...
    for (const s of (d.sessions || [])) {
      db.prepare(`INSERT OR REPLACE INTO sessions (id, subgroupId, date, time, status, note)
                  VALUES (?, ?, ?, ?, ?, ?)`)
    ...
    const blobTables = [
      ["subgroup_messages", d.subgroupMessages],
...
function seedDefaults() {
  const pw = hashPasswordSync("admin1234");
  db.transaction(() => {
    ...
    db.prepare("INSERT OR IGNORE INTO lang_levels (id, nom) VALUES ('ll1','A1'),('ll2','A2'),('ll3','B1')").run();
