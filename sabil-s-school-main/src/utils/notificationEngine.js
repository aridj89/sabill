...
function createNotification(userId, type, title, message) {
  return {
    id: uid(),
    userId,
    type, // 'info', 'payment', 'presence', 'session'
    title,
    message,
...
export function notifyGroupMessage(data, subgroupId) {
  const students = data.students.filter(s => s.subgroupId === subgroupId);
  const newNotifs = students.map(st => createNotification(st.id, "info", "Nouveau message", "Le professeur a envoyé un nouveau message dans le groupe."));
  return [...(data.userNotifications || []), ...newNotifs];
}
 ...
 */
export function notifyAdminPrivateMessage(data, studentName) {
  const notif = createNotification("admin", "info", "Nouveau message privé", `${studentName} vous a envoyé un message privé.`);
  return [...(data.userNotifications || []), notif];
}
...
export function notifyStudentPrivateMessage(data, studentId) {
  const notif = createNotification(studentId, "info", "Nouveau message", "L'administration vous a envoyé un message privé.");
  return [...(data.userNotifications || []), notif];
}
