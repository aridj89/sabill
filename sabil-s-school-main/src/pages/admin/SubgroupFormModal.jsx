...
export default function SubgroupFormModal({
  catId, levelId, groupType,    // context (where we are in hierarchy)
  ...
  const isValid = form.nom.trim() && form.days.length > 0 && form.startDate && form.endDate;

  const catColor = cat?.color || C.accent;
  const title = initial
    ? (lang === "ar" ? "تعديل المجموعة" : "Modifier le sous-groupe")
    : (lang === "ar" ? "مجموعة جديدة" : "Nouveau sous-groupe");
  ...
  return (
    <Modal title={title} onClose={onClose} wide>
      ...
      {/* Nom */}
      <Field label={lang === "ar" ? "اسم المجموعة" : "Nom du sous-groupe"}>
        <input
          autoFocus
          style={inputStyle}
          placeholder={lang === "ar" ? "مثال: مجموعة 1" : "Ex: Groupe 1, Groupe avancé…"}
          value={form.nom}
      ...
      <div style={{ marginTop: 16 }}>
        <PrimaryBtn full onClick={handleSave} disabled={!isValid}>
          {initial
            ...
            : (lang === "ar" ? "إنشاء المجموعة" : "Créer le sous-groupe")}
        </PrimaryBtn>
      </div>
    </Modal>
  );
